...
def main(_run, _config, _log):
    ...
    for i_iter, sample_batched in enumerate(trainloader):
        # Prepare input
        support_images = [[shot.cuda() if _config['gpu_id'] else shot for shot in way]
                          for way in sample_batched['support_images']]
        support_fg_mask = [[shot[f'fg_mask'].float().cuda() if _config['gpu_id'] else shot[f'fg_mask'].float() for shot in way]
                           for way in sample_batched['support_mask']]
        support_bg_mask = [[shot[f'bg_mask'].float().cuda() if _config['gpu_id'] else shot[f'bg_mask'].float() for shot in way]
                           for way in sample_batched['support_mask']]

        query_images = [query_image.cuda() if _config['gpu_id'] else query_image
                        for query_image in sample_batched['query_images']]
        query_labels = torch.cat(
            [query_label.long().cuda() if _config['gpu_id'] else query_label.long() for query_label in sample_batched['query_labels']], dim=0)
